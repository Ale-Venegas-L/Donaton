...
public class DonationController {
    ...
    public ResponseEntity<Donation> update(
            ...
            @Parameter(description = "Información actualizada de la donación") @RequestBody Donation donation) {
        if (id == null) {
            throw new IllegalArgumentException("Donation ID cannot be null");
        }
        if (donation == null) {
        ...
        if (optionalDonation.isPresent()) {
            ...
            donationToUpdate.setDonorName(donation.getDonorName());
            donationToUpdate.setDescription(donation.getDescription());
    ...
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Donación eliminada exitosamente"),
        @ApiResponse(responseCode = "404", description = "Donación no encontrada")
    })
    @DeleteMapping("/{id}")
